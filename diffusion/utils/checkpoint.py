...
def save_checkpoint_ddp(
    ...
    limit = 1 if keep_last else checkpoint_total_limit
    if limit is not None:
        ...
        for old_ckpt in existing[:-limit] if limit > 0 else existing:
            os.remove(old_ckpt)
    if add_symlink:
        ...
        if limit is not None:
            ...
            checkpoints = sorted(
                (
                    ...
                    if os.path.isdir(os.path.join(work_dir, d)) and d.startswith("epoch_")
                ),
                key=os.path.getmtime,
            )
            for old_dir in (checkpoints[:-limit] if limit > 0 else checkpoints):
